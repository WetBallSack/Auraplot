...
import { ChevronLeft, ChevronRight, Plus, Calendar as CalendarIcon, X, Trash2, Check, TrendingUp, TrendingDown, Copy, Loader2, Info, Lock, Crown } from 'lucide-react';
...
export const Planner: React.FC = () => {
    ...
    const handleAddItem = async (e: React.FormEvent) => {
        ...
        try {
            const newOrder = await api.saveScheduledOrder({
                ...
                // Stickiness isn't strictly part of ScheduledOrder yet in schema but generic logic allows it
                // We map it to impact weight conceptually or if schema supports it.
                // Assuming schema update might be needed or handled implicitly.
                // Since ScheduledOrder interface doesn't have stickiness, we might lose it unless schema matches.
                // For now we persist what we can. Ideally update types.
    ...
    const handleDeployStrategy = async (strat: Strategy) => {
        ...
        if (!user?.isPro && (dailyOrders.length + strat.orders.length > 7)) {
             alert("Cannot deploy: This would exceed the 7-item daily limit for free accounts.");
             return;
        }
    ...
    return (
        <div className="w-full flex gap-8 h-[600px]">
            {/* Calendar Grid */}
            <div className="flex-1 bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-zinc-800 p-6 flex flex-col">
                    ...
                    {getDaysInMonth().map((day, idx) => {
                        ...
                        return (
                            <div 
                                ...
                                className={clsx(
                                    "rounded-xl border p-2 relative cursor-pointer transition-all hover:border-primary/50 flex flex-col justify-between group",
            ...
            <AnimatePresence mode="wait">
                {selectedDateStr ? (
                    <motion.div 
                        ...
                        exit={{ opacity: 0, x: 20 }}
                        className="w-[320px] bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-zinc-800 p-6 flex flex-col shadow-xl"
                    >
                        <div className="flex justify-between items-center mb-6">
                                    ...
                                    <div key={o.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-100 dark:border-zinc-800">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 text-[10px] text-gray-400">
                                                {o.time && <span className="font-mono bg-gray-200 dark:bg-zinc-700 px-1 rounded">{o.time}</span>}
                        ...
                        {!user?.isPro && (
                            <div className="mb-4 text-xs flex justify-between items-center bg-gray-50 dark:bg-zinc-800 px-3 py-2 rounded-lg">
                                <span className="text-gray-500">Daily Slots Used</span>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative">
                                ...
                                <AnimatePresence>
                                    {showStratMenu && (
                                        ...
                                        >
                                            <div className="text-[10px] font-bold text-gray-400 uppercase px-2 py-1">Select Template</div>
                                            <div className="bg-blue-50 dark:bg-blue-900/20 px-2 py-2 mb-2 rounded-lg text-[10px] text-blue-600 dark:text-blue-300 leading-tight">
                                                Create reusable templates in the "Live Checklist" tab to quickly populate days here.
                                            </div>
                                            {strategies.map(s => (
                                                <button 
                                                    key={s.id}
                                            ...
                                            {strategies.length === 0 && <div className="px-2 py-2 text-xs text-gray-400">No templates found.</div>}
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            </div>
                            ...
                            <form onSubmit={handleAddItem} className="bg-gray-50 dark:bg-zinc-800/30 p-3 rounded-xl border border-gray-100 dark:border-zinc-800">
                                ...
                                <div className="space-y-3 mb-4">
                                    <div className="flex justify-between text-[10px] font-medium">
                                        <span className={newItemImpact < 0 ? "text-danger" : "text-gray-400"}>Drain (-10)</span>
                                        <span className="font-mono font-bold text-gray-800 dark:text-white">{newItemImpact > 0 ? '+' : ''}{newItemImpact}</span>
                                        <span className={newItemImpact > 0 ? "text-primary" : "text-gray-400"}>Boost (+10)</span>
                                    </div>
                                    <input 
                                        type="range"
                                ...
                                <div className="grid grid-cols-2 gap-2 mb-3">
                                    <div>
                                        <label className="text-[10px] text-gray-400 block mb-1">Intensity {newItemIntensity}</label>
                                        <input 
                                            type="range"
                                            min="1" max="10"
                                    ...
                                    <div>
                                        <label className="text-[10px] text-gray-400 block mb-1">Stickiness {newItemStickiness}</label>
                                        <input 
                                            type="range"
                                ...
                                <div className="flex gap-2 mb-3">
                                    <input 
                                        type="time" 
                                        className="bg-transparent text-xs outline-none text-gray-500 dark:text-gray-400 w-full"
                                        value={newItemTime}
                                ...
                                >
                                    {!user?.isPro && dailyOrders.length >= 7 ? (
                                        ...
                                        `Add to ${new Date(selectedDateStr).getDate()}th`
                                    )}
                                </button>
                            </form>
                            ...
                            <div className="flex items-start gap-2 text-[10px] text-gray-400 leading-tight pt-1">
                                ...
                                Items added here will appear in your Live Checklist on the scheduled day.
                            </div>
                        </div>
                    </motion.div>
                ) : (
                    <motion.div 
                        ...
                        initial={{ opacity: 0 }} 
                        animate={{ opacity: 1 }}
                        className="w-[320px] bg-gray-50 dark:bg-zinc-900/50 rounded-3xl border border-dashed border-gray-200 dark:border-zinc-800 flex flex-col items-center justify-center text-gray-400 p-6"
                    >
                        <CalendarIcon size={48} className="mb-4 opacity-20" />
                        <p className="text-sm font-medium text-center">Select a date to plan your market moves.</p>
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
